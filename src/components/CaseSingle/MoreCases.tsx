...
function CasesMain({ caseData }: { caseData: Casetype }) {
	const cases = [
		{
			title: 'Al Jazeera',
	...
	return (
		<section className={`${S.otherCases} pt-16`}>
			...
			<div className='grid grid-cols-2 2xl:grid-cols-4 gap-4 px-4 mt-16 mb-4'>
				{cases.map(({ title, image, url }) => (
					<CaseCard key={title} title={title} image={image} url={url} active={caseData.url === url} />
				))}
			</div>
		</section>
	)
